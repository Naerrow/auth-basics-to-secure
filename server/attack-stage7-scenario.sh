...
HEADER_FILE="/tmp/attack_stage7_headers.txt"
BODY_FILE="/tmp/attack_stage7_body.txt"
CODE_FILE="/tmp/attack_stage7_code.txt"
...
log "Stage7 취약점 검증 시나리오 시작 (Base: ${BASE_URL})"

log "0) Stage7 CSRF 컨텍스트 준비"
...
if [ -z "$REFRESH_COOKIE" ]; then
  ...
  log "7) Stage7: refresh 토큰 회전(재사용 즉시 폐기)"
  http_call 1 "${CSRF_COOKIE_NAME}=${CSRF_COOKIE}" -X POST "${BASE_URL}/login" \
    -H 'Content-Type: application/json' \
    -d '{"username":"demo","password":"demo"}'
  capture_response
  ST7_REFRESH_COOKIE="$(extract_refresh_cookie "$HEADER_FILE")"

  if [ -z "$ST7_REFRESH_COOKIE" ]; then
    not_testable "refresh 회전 구현" "회전 테스트용 refresh 발급 실패"
  else
    ST7_CODE1=$(mktemp)
    ST7_CODE2=$(mktemp)
    ST7_CODE3=$(mktemp)
    ST7_REFRESH_HEADER="/tmp/attack_stage7_refresh_rotate_headers.txt"
    ST7_REFRESH_BODY="/tmp/attack_stage7_refresh_rotate_body.txt"
    ST7_BODY2="/tmp/attack_stage7_refresh_rotate_body2.tmp"
    ST7_BODY3="/tmp/attack_stage7_refresh_rotate_body3.tmp"

    # 1회차: 최초 refresh 사용
    (
      rm -f "$ST7_REFRESH_HEADER" "$ST7_REFRESH_BODY"
      curl -sS -D "$ST7_REFRESH_HEADER" -o "$ST7_REFRESH_BODY" -w "%{http_code}" \
        -X POST "${BASE_URL}/auth/refresh" \
        -H "Origin: ${CLIENT_ORIGIN_HEADER}" \
        -H "Referer: ${CLIENT_REFERER_HEADER}" \
        -H "X-CSRF-Token: ${CSRF_TOKEN}" \
        -H "Cookie: ${CSRF_COOKIE_NAME}=${CSRF_COOKIE}; refresh_token=${ST7_REFRESH_COOKIE}"
    ) > "$ST7_CODE1"
    ST7_STATUS1="$(cat "$ST7_CODE1")"

    if [ "$ST7_STATUS1" != "200" ]; then
      vulnerable "refresh 회전 구현" "1회차 재발급이 HTTP ${ST7_STATUS1}로 실패"
    else
      ST7_NEW_REFRESH_TOKEN="$(extract_refresh_cookie_from_file "$ST7_REFRESH_HEADER")"
      if [ -z "$ST7_NEW_REFRESH_TOKEN" ]; then
        not_testable "refresh 회전 구현" "재발급 응답에서 새 refresh 토큰 재발급 쿠키 부재"
      else
        # 2회차: 동일한 기존 토큰 재사용 시 401/403/409/429 기대
        (
          rm -f "$ST7_BODY2"
          curl -sS -o "$ST7_BODY2" -w "%{http_code}" \
            -X POST "${BASE_URL}/auth/refresh" \
            -H "Origin: ${CLIENT_ORIGIN_HEADER}" \
            -H "Referer: ${CLIENT_REFERER_HEADER}" \
            -H "X-CSRF-Token: ${CSRF_TOKEN}" \
        ...
        ST7_STATUS2="$(cat "$ST7_CODE2")"

        if [ "$ST7_STATUS2" = "200" ]; then
          vulnerable "refresh 회전 미구현" "동일 refresh 토큰으로 재사용 재발급이 HTTP 200 (재사용 허용)"
        elif [ "$ST7_STATUS2" = "401" ] || [ "$ST7_STATUS2" = "403" ] || [ "$ST7_STATUS2" = "409" ] || [ "$ST7_STATUS2" = "429" ]; then
          # 3회차: 새 refresh 토큰이 유효한지(일부 구현은 재사용 탐지 즉시 패밀리 폐기) 확인
          (
            rm -f "$ST7_BODY3"
            curl -sS -o "$ST7_BODY3" -w "%{http_code}" \
              -X POST "${BASE_URL}/auth/refresh" \
              -H "Origin: ${CLIENT_ORIGIN_HEADER}" \
              -H "Referer: ${CLIENT_REFERER_HEADER}" \
              -H "X-CSRF-Token: ${CSRF_TOKEN}" \
              -H "Cookie: ${CSRF_COOKIE_NAME}=${CSRF_COOKIE}; refresh_token=${ST7_NEW_REFRESH_TOKEN}"
          ) > "$ST7_CODE3"
          ST7_STATUS3="$(cat "$ST7_CODE3")"

          if [ "$ST7_STATUS3" = "200" ]; then
            secure "refresh 회전 구현" "기존 refresh 재사용 차단(${ST7_STATUS2}), 새 refresh는 재발급 성공(${ST7_STATUS3})"
          elif [ "$ST7_STATUS3" = "401" ] || [ "$ST7_STATUS3" = "403" ] || [ "$ST7_STATUS3" = "429" ] || [ "$ST7_STATUS3" = "409" ]; then
            secure "refresh 회전 구현" "기존 refresh 재사용 차단(${ST7_STATUS2}) 후 패밀리 정책으로 재발급도 차단(${ST7_STATUS3})"
          else
            vulnerable "refresh 회전 구현" "재사용 차단은 되나 새 refresh 처리 결과가 비정상: 3차=${ST7_STATUS3}"
          fi
        else
          vulnerable "refresh 회전 구현" "재사용 결과 코드 판정 불가: 2차=${ST7_STATUS2}"
        fi
      fi
    fi

    rm -f "$ST7_CODE1" "$ST7_CODE2" "$ST7_CODE3" "$ST7_REFRESH_HEADER" "$ST7_REFRESH_BODY" "$ST7_BODY2" "$ST7_BODY3"
  fi
fi
...
if [ "$VULNERABLE_COUNT" -gt 0 ]; then
  echo "[SUMMARY] Stage7에서 재현 가능한 취약점이 확인되었습니다."
  exit 2
fi
echo "[SUMMARY] Stage7 기준 즉시 재현 취약점이 없습니다."
